...
@push('manage-facilities-reservation-table-scripts')
<script type="text/javascript">
    document.addEventListener('DOMContentLoaded', function () {
        ...
        window.livewire.on('openFacilitiesReservationModal', () => {
            $('#FacilitiesReservationModal').modal('show')
        });
        
        window.livewire.on('closeFacilitiesReservationModal', () => {
            $('#FacilitiesReservationModal').modal('hide')
        });
        ...
        window.livewire.on('openSwalDelete', (FacilitiesReservationID) => {
            swal("Are you sure you want to delete this Data?", {
                dangerMode: true,
                buttons: true,
            }).then(okay => {
                if (okay) {
                    window.Livewire.emit('DeleteData', FacilitiesReservationID)
                }else{
                    window.Livewire.emit('refresh_schoolfacilities_table')
                }
                });
        });
