...
@push('school-bus-rental-table-scripts')
<script type="text/javascript">
    document.addEventListener('DOMContentLoaded', function () {
        ...
        window.livewire.on('openSchoolBusRentalModal', () => {
            $('#SchoolBusRentalModal').modal('show')
        });
        
        window.livewire.on('closeSchoolBusRentalModal', () => {
            $('#SchoolBusRentalModal').modal('hide')
        });
        
        
        
        window.livewire.on('openSwalDelete', (SchoolBusRentalID) => {
            swal("Are you sure you want to delete this Data?", {
                dangerMode: true,
                buttons: true,
            }).then(okay => {
                if (okay) {
                    window.Livewire.emit('DeleteData', SchoolBusRentalID)
                }else{
                    window.Livewire.emit('refresh_schoolbusrental_table')
                }
                });
        });
